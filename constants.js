...
const INTERESTED_ACTIONS = ['Contacted', 'Send WhatsApp', 'Call Again'];
const NOT_REACHABLE_ACTIONS = ['No Answer', 'Switched Off', 'No Answer - Multiple Times'];
const WARM_ACTIONS = ['Interest in Resale', 'Interest in Separate'];
...
export const clientStatus = (c) => {
  ...
  if (c.call_result && WARM_ACTIONS.includes(c.call_result))       return { label: 'Warm Lead',      color: '#f59e0b' };
  if (c.call_result && INTERESTED_ACTIONS.includes(c.call_result)) return { label: 'Interested',     color: '#6366f1' };
  if (c.call_result && NOT_REACHABLE_ACTIONS.includes(c.call_result)) return { label: 'Not Reachable', color: '#8B93A3' };
  if (c.ever_contacted)                                            return { label: 'Interested',     color: '#6366f1' };
